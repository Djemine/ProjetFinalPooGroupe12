...
class ChauffeursView(ctk.CTkFrame):
    def __init__(self, parent, api_client):
        super().__init__(parent, fg_color="transparent")
        self.api_client = api_client

        ctk.CTkLabel(self, text="Gestion des chauffeurs", font=POLICE_TITRE,
        ...
        ctk.CTkLabel(formulaire, text="Ajouter un chauffeur", font=POLICE_SOUS_TITRE).grid(
            row=0, column=0, columnspan=5, sticky="w", padx=16, pady=(14, 8))

        self.e_nom = ctk.CTkEntry(formulaire, placeholder_text="Nom", width=140)
        self.e_nom.grid(row=1, column=0, padx=8, pady=(0, 14))
        self.e_prenom = ctk.CTkEntry(formulaire, placeholder_text="Prénom", width=140)
        self.e_prenom.grid(row=1, column=1, padx=8, pady=(0, 14))
        self.e_tel = ctk.CTkEntry(formulaire, placeholder_text="Téléphone", width=140)
        self.e_tel.grid(row=1, column=2, padx=8, pady=(0, 14))
        self.e_permis = ctk.CTkEntry(formulaire, placeholder_text="N° permis", width=130)
        self.e_permis.grid(row=1, column=3, padx=8, pady=(0, 14))
        self.e_immat = ctk.CTkEntry(formulaire, placeholder_text="Immatriculation bus", width=160)
        self.e_immat.grid(row=1, column=4, padx=8, pady=(0, 14))
        ctk.CTkButton(formulaire, text="Ajouter", command=self.ajouter_chauffeur,
        ...
        ctk.CTkLabel(self, text="Chauffeurs enregistrés", font=POLICE_SOUS_TITRE).pack(
    ...
    def ajouter_chauffeur(self):
        valeurs = {
            ...
            "telephone": self.e_tel.get().strip(), "numero_permis": self.e_permis.get().strip(),
            "immatriculation_bus": self.e_immat.get().strip(),
        }
        ...
        try:
            self.api_client.creer_chauffeur(**valeurs)
        except ApiError as e:
            self.label_erreur.configure(text=str(e))
            return

        self.label_erreur.configure(text="")
        for champ in (self.e_nom, self.e_prenom, self.e_tel, self.e_permis, self.e_immat):
            champ.delete(0, "end")
        self.rafraichir()
    ...
    def rafraichir(self):
        ...
        try:
            chauffeurs = self.api_client.lister_chauffeurs()
        except Exception:
            return

        if not chauffeurs:
            ctk.CTkLabel(self.zone_liste, text="Aucun chauffeur enregistré pour le moment",
        ...
        for c in chauffeurs:
            ...
            ctk.CTkLabel(ligne, text=f"{c['prenom']} {c['nom']}", width=220, anchor="w",
                         font=POLICE_TEXTE).pack(side="left")
            ctk.CTkLabel(ligne, text=c["telephone"], width=150, anchor="w",
                         font=POLICE_TEXTE).pack(side="left")
            ctk.CTkLabel(ligne, text=c["numero_permis"], width=150, anchor="w",
                         font=POLICE_TEXTE).pack(side="left")
            ctk.CTkLabel(ligne, text=c["immatriculation_bus"] or "—", width=180, anchor="w",
                         font=POLICE_TEXTE).pack(side="left")